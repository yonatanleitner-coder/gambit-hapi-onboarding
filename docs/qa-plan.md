...
| 4 | Browser happy-path (Playwright) | **Not yet built — AI Plan §12 task 10.** Planned: derive from `legal_two_team`, drive the deployed (or local, `PROVIDER=mock`) UI, assert the GUI panel and chat both show the same final state and a legal verdict. This is the brief's "bonus: one automated browser test" and the only automated check that exercises the frontend, not just the API. | | |
...
**Honest gap:** #4 is planned, not built. Until it exists, the only thing exercising the actual rendered frontend is manual QA below plus the one-time Playwright-driven visual pass done during frontend build (task 8, see `docs/end-of-session.md`) — that pass is not a regression test; it doesn't re-run.
...
- **No automated cross-browser matrix** — Playwright (once built) targets one engine/viewport; manual check #11/#12 is the only cross-viewport/theme coverage.
